...
#include "mathcore/src/random/ssRandom.h"
...
private:
  ...
  {
    CrossoverSegment(int start_idx_, int end_idx_)
    {
  ...
  static ssRandom m_random;
  std::vector<int> m_crossover_pts;
  std::vector<CrossoverSegment> m_crossover_segments;

public:
  enum NoiseFunc { UniformUnity, UniformPlusMinusOne };
  ...
  static Chromosome()
  {
    tm time_data;
    time(&time_data);
    m_random.setQuickRandSeed(time.tm_sec);
  }

  Chromosome(int num_basepairs, NoiseFunc noise_func)
  {
  ...
  Chromosome(float[] newdna, NoiseFunc noise_func)
  {
    m_noise_func = noise_func;
...
private:
  ...
  // Crossover
  std::vector<Chromosome> Mate(Chromosome partner, int num_crossover_pts, bool two_children)
  {
